...
import { isLogin, STATUS_LOGIN } from '../../util.js'
...
async function init() {
    ...
    const { status, data } = await isLogin();
    if (status === STATUS_LOGIN.FAILED) {


        const content = `
        <div class="flex justify-center align-middle self-center" id="mensaje_error">
            <p class="msg">Favor de Iniciar Sesión</p>
        </div>
    `
        ...
        setTimeout(() => {
            const msg_error = document.querySelector('#mensaje_error');

            msg_error.innerHTML = `
                <p class="msg_2">Redirigiendo a Login...</p>
            `
        }, 500);
...
async function loadMuscles() {
    try {
        ...
        state.muscles = [
            { musculo: 'Pecho', imagen: '/pecho.png' },
            { musculo: 'Bíceps', imagen: '/bicep.png' },
            { musculo: 'Espalda', imagen: '/espalda.png' }
        ];
    }
    renderMuscles();
}
    ...
    ejercicios.forEach(e => {
        ...
        <div class="exercise-body">
          ...
          <div class="card-images mb-3">
            <img src="${escapeHtml(e.imagen_inicio || e.imagen_inicio_url || e.imageninicio_url || '')}" alt="inicio" onerror="this.src='/fallback.png'"/>
            <img src="${escapeHtml(e.imagen_fin || e.imagen_fin_url || e.imagenfin_url || '')}" alt="fin" onerror="this.src='/fallback.png'"/>
          </div>
...
function mockEjercicios(muscle) {
    ...
    return [
        { id_ejercicio: base + 1, nombre: `${muscle} - Press`, tipo_ejercicio: 'basico', musculo: muscle, imagen_inicio: '/fallback.png', imagen_fin: '/fallback.png', descripcion: 'Descripción de ejemplo' },
